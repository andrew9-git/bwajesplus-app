...
if(isset($_POST['id']))
{
    ...
    if($publish == 0)
    {
        ...
        $no_of_private_post_allowed = fetch_single_row($id, 'users')['no_of_private_post_allowed'];
        if(db_row_count($id, 'user_id', 'posts', 'int') >= 10)
        {
    ...
    else
    {
        ...
        {
            $errors[] = 'You must upload a cover photo';
        }
    }
    ...
    if(empty($errors))
    {
        ...
        $executed = insert_into_posts($values);

        if($executed)
        {
            $msg = "<div class='card success'><div><b>Success!</b> Your post has been created</div></div>";
            echo $msg;
        }
