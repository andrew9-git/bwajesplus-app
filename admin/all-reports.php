...
<div class="home-content">
            ...
            <form action="">
                <div class="form-wrapper">
                    <div class="form-group">
                        <span><b>Total reports - <span id="total_reports"></span></b></span>
                    </div>
            ...
            if(this.status == 200)
            {
                ...
                if(response.data.length > 0)
                {
                    for(let count = 0; count < response.data.length; count++)
                    {
                      ...
                      html += '<td><a href="edit-report/'+ response.data[count].report_id +'"><i class="bx bx-link-external"></i></a><?php if($admin['admin_type'] == 1){ ?>|<span style="cursor: pointer;" onclick="event.preventDefault();if(confirm(&quot;Do you really want to delete this report?&quot;)){document.getElementById(&quot;form-delete-'+ response.data[count].report_id +'&quot;).submit();}"><i class="bx bx-trash"></i></span><form method="post" action="all-reports" style="display: none;" id="form-delete-'+ response.data[count].report_id +'"><input type="hidden" value="'+ response.data[count].report_id +'" name="delete-report" class="form_data_dr"></form><?php } ?></td>';
                      html += '</tr>';
                      serial_no++;
    ...
    <?php
      if(isset($_POST['delete-report']))
      {
        ...
        $executed = delete_single_row($report_id, 'reports');
        if($executed)
        {
          $url = $host . 'all-reports';
          redirect_to($url);
        }
      }
    ?>
